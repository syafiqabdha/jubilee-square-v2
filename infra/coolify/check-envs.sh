...
# docker-compose.yml declares every secret with `${VAR:?}` and no fallback, so
# this check mirrors what the stack will refuse to start without.
set -euo pipefail

REPO_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
ENV_FILE="${1:-$REPO_ROOT/.env}"
[ -f "$ENV_FILE" ] && ENV_FILE="$(cd "$(dirname "$ENV_FILE")" && pwd)/$(basename "$ENV_FILE")"
...
# 1. Duplicate keys — the first definition wins in dotenv parsing, so a stale
#    value earlier in the file silently shadows the intended one.
...
# 2. Required secrets — no defaults exist in docker-compose.yml.
REQUIRED_SECRETS=(DB_PASSWORD KEY SECRET ADMIN_PASSWORD)
# Values published in the repository/git history before the PAN-45 S-3 fix, and
# the placeholders shipped in .env.example. Neither may reach a deployment.
BANNED_VALUES=(
...
for key in "${REQUIRED_SECRETS[@]}"; do
  ...
  if [ -z "$value" ]; then
    fail "$key is missing or empty — docker-compose.yml requires it (generate: openssl rand -hex 32)."
  fi
  ...
  if [ "${#value}" -lt 16 ]; then
    echo "⚠️ $key is shorter than 16 characters — consider openssl rand -hex 32."
  fi
